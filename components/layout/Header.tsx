      ...
      <nav className="bg-white shadow-sm border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            {NAV_LINKS.map((link) =>
              link.children ? (
                ...
                >
                  <button className="text-sm font-medium text-gray-700 hover:text-[#0066CC] flex items-center gap-1">
                    {link.label}
                    ...
                    <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-gray-100 rounded-lg shadow-lg py-1 z-50">
                      {link.children.map((child) => (
                        <Link
                          key={child.href}
                          href={child.href}
                          className="block px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-[#0052a3]"
                        >
                          {child.label}
                        </Link>
                      ))}
                ...
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm font-medium text-gray-700 hover:text-[#0066CC]"
                >
                  {link.label}
                </Link>
              )
          ...
          <a
            ...
            className="hidden md:inline-flex items-center px-4 py-2 bg-[#0066CC] hover:bg-[#0052a3] text-white font-semibold text-sm rounded-lg transition-colors"
          >
            Request a Quote
          </a>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-100 bg-white px-4 pb-4">
            {NAV_LINKS.map((link) =>
              link.children ? (
                <div key={link.label}>
                  ...
                  {link.children.map((child) => (
                    <Link
                      key={child.href}
                      href={child.href}
                      className="block py-2 pl-3 text-sm text-gray-700 hover:text-[#0066CC]"
                ...
                <Link
                  key={link.href}
                  href={link.href}
                  className="block py-2 text-sm font-medium text-gray-700 hover:text-[#0066CC]"
                  onClick={() => setMenuOpen(false)}
                >
                  {link.label}
                </Link>
            ...
            <a
              href={QUOTE_URL}
              className="mt-4 block text-center px-4 py-3 bg-[#0066CC] text-white font-semibold text-sm rounded-lg"
            >
              Request a Quote
            </a>
          </div>
